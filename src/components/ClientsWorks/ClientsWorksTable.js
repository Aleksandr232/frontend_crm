...
const ClientsWorksTable=()=>{
    ...
    const handleSubmitAct = (e) => {
      ...
      axios.post(`https://cs65705.tw1.ru/api/clients/act/${selectedClientAct}`, formData, {
          headers: {
              'Authorization': `Bearer ${token}`
          }
      })
      ...
      const delClient = () => {
          const token = localStorage.getItem("token");
          axios.delete(`https://cs65705.tw1.ru/api/clients/${selectedClient.id}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
            }
          })
      ...
      const fetchData = () => {
        const token = localStorage.getItem("token");
        axios.get(`https://cs65705.tw1.ru/api/clients/all`, {
          headers: {
            Authorization: `Bearer ${token}`
          }
        })
      ...
      const wordExport = () => {
        ...
        axios.post(`https://cs65705.tw1.ru/api/clients/document/${id}`, formData, {
            headers: {
                Authorization: `Bearer ${token}`,
            }
        })
        ...
        <div className="fixed z-10 inset-0 flex items-center justify-center">
          ...
          <button
            ...
            {selectedClient.path_act ?
              (<div>
                <a href={`https://cs65705.tw1.ru/document/${selectedClient.path_act}`}>
